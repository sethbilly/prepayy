...
class UserPasswordChangeRequested extends Notification implements ShouldQueue
{
    use Queueable;
    /**
     * @var string
...
NOTE;
        ...
        return (new MailMessage)
            ->subject(sprintf(
                'Request from Admin %s to Change Your Account Passowrd on %s',
                $notifiable->institutable->name ?? 'Callens', config('app.name')
            ))
            ->greeting("Hi {$notifiable->firstname}!")
            ->line(sprintf('The admin of %s on %s requests that you change your account password immediately',
                $notifiable->institutable->name ?? 'Callens', config('app.name'))
            )
            ->line('To set a new account password, click the link below')
