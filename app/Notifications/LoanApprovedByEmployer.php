...
class LoanApprovedByEmployer extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ...
            ->line(sprintf(
                '%s has approved your application for a %s loan from %s',
                $this->approver->institutable->name,
                $this->application->loanProduct->name,
                $this->application->loanProduct->institution->name
            ))
            ->line(sprintf(
                'Click the link below to review your application or submit it to %s',
                $this->application->loanProduct->institution->name
            ))
